        ...
        <x-card class="mx-sm-auto shadow-0" style="margin-top:70px;max-width: 500px;background-color: #f7f9fb">
           <x-flash-message :redirect="url('')"/>
            ...
            <p class="fw-bold text-center">
              <small>Post a gig to find a developer</small>
            </p>
            <form method="POST" action="/listings" enctype="multipart/form-data">
              @csrf
              <div class="form-group mb-4">
                <label>Company</label>
                <input name="company" type="text" class="form-control" placeholder="" value="{{old('company')}}">
                @error('company')
              ...
              <div class="form-group mb-4">
                ...
                <input name="title" type="text" class="form-control" placeholder="" value="{{old('title')}}">
                @error('title')
                <small class="text-danger">{{$message}}</small>
                @enderror
              </div>
              ...
              <div class="form-group mb-4">
                ...
                <input name="location" type="text" class="form-control" placeholder="" value="{{old('location')}}">
                @error('location')
                <small class="text-danger">{{$message}}</small>
                @enderror
              </div>
              <div class="form-group mb-4">
                ...
                <input name="website" type="text" class="form-control" placeholder="" value="{{old('website')}}">
                @error('website')
                <small class="text-danger">{{$message}}</small>
                @enderror
              </div>
              <div class="form-group mb-4">
                ...
                <input name="tags" type="text" class="form-control" placeholder="Example:PHP,Laravel,CMS" value="{{old('tags')}}">
                @error('tags')
                <small class="text-danger">{{$message}}</small>
                @enderror
              </div>
